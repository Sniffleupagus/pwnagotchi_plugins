...
class Miyagi(plugins.Plugin):
        ...
        try:
            if request.method == "GET":
                if path == "/" or not path:
                    ...
                    secnum = 0
                    for sec in [self.agent._config['ai'],
                                self.agent._config['ai']['params'],
                                 self.agent._config['personality']]:
                        secnum += 1
                        ret += '<tr><th colspan=2>Section %s</th></tr>' % secnum
    ...
    def on_loaded(self):
        logging.warning("PWN on, PWN off! " % self.options)
    ...
    def on_ready(self, agent):
        ...
        if self.agent._config['ai']['laziness'] > 0.95:
    ...
    def on_ai_training_step(self, agent, _locals, _globals):
        self._train_epoch = self._train_epoch + 1
    ...
    def on_ai_training_end(self, agent):
        ...
        if self.agent._config['ai']['laziness'] < 0.8:
            self.agent._config['ai']['laziness'] *= 0.8
            self.agent._config['ai']['laziness'] += 0.2
        else:
            self.agent._config['ai']['laziness'] *= 0.9
            self.agent._config['ai']['laziness'] += 0.1
